...
\subsection{Empresa y contexto de negocio}

Enmarqué el proyecto en \textbf{Nexus Sentinel}, una empresa ficticia de tecnología de tamaño medio
que sufrió un incidente representativo del problema: la cuenta de uno de sus ingenieros fue utilizada
durante semanas para explorar servidores a los que ese ingeniero jamás accedía. Ninguna alerta se
disparó, porque cada autenticación individual era «válida». El incidente se detectó tarde y de forma
casi accidental. Este caso ficticio ---que, como mostraré en la presentación, tiene equivalentes
reales y costosos en la industria--- define con precisión el problema que mi sistema busca resolver:
no vigilar eventos individuales, sino la \textbf{desviación del patrón de comportamiento}.

\subsection{Flujo de trabajo}

El desarrollo que documento comprende ocho fases: (1) adquisición y muestreo justificado de los datos
crudos; (2) análisis exploratorio orientado a descubrir empíricamente las señales de compromiso;
(3) ingeniería de variables en cuatro niveles de desviación; (4) modelado supervisado y no supervisado
con evaluación y análisis de estabilidad; (5) construcción del \textit{pipeline} de inferencia y la
puntuación de riesgo; (6) exposición de los modelos mediante una API; (7) desarrollo de una interfaz
web con un asistente conversacional; y (8) la redacción de este documento. Cada fase la cerré con una
verificación explícita, y varias de ellas revelaron conflictos técnicos y metodológicos que
documento con transparencia, pues considero que la forma en que los resolví es parte sustancial del
valor del trabajo.
...
prueba lo reservé para una única evaluación final.
...
producción. Extraigo varias conclusiones.

En lo metodológico, confirmé que la elección del aprendizaje supervisado está sólidamente justificada
para este problema: dado que dispongo de etiquetas verificadas del \textit{red team}, puedo optimizar
...
fraude genuino de comportamiento legítimo atípico--- no permitiría. No obstante, integré ambos
paradigmas: el modelo no supervisado aporta la línea base y, vía el agrupamiento de pares, genera las
variables de desviación grupal que resultaron valiosas.

En lo técnico, el hallazgo que considero más relevante es que la señal del uso indebido de
credenciales vive en la \textbf{desviación} y en la \textbf{novedad de conexiones}, no en el volumen
crudo, tal como confirmó el análisis SHAP. La ingeniería de variables en cuatro niveles ---y en
particular la regla anti-fuga que la sostiene--- es, en mi valoración, la contribución técnica central
del trabajo.

En cuanto a los resultados, fui deliberadamente honesto: son modestos en términos absolutos (un
PR-AUC de prueba de 0.056) pero representan una discriminación real y estable, muy por encima del
azar, sobre un problema con solo 181 eventos de compromiso y un desbalance extremo. La curva de
alertas por día frente a recall demuestra que, aun con estas métricas, el sistema aporta valor
operativo: reduce el pajar a una lista corta y priorizada. La utilidad plena se consolidaría con la
línea de trabajo futuro más prometedora ---y la única que añadiría señal genuina en lugar de reordenar
la existente--- que es el enriquecimiento con fuentes de datos adicionales del propio conjunto de LANL
(procesos y flujos de red), contempladas como opcionales en el diseño original.

Finalmente, valoro que los conflictos que enfrenté ---el \textit{data fence} en la adquisición, el
agrupamiento degenerado y, sobre todo, la autocorrección de la fuga metodológica--- no fueron
obstáculos que ocultar, sino oportunidades para demostrar el rigor con el que abordé el proyecto. La
forma en que un sistema falla y se corrige dice tanto de su calidad como sus métricas.
